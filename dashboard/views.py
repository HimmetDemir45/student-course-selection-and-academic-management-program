...
class ApproveStudentView(LoginRequiredMixin, View):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if action == "reject":
            ...
            messages.warning(
                request,
                f"{profile.user.get_full_name() or profile.user.username} reddedildi ve hesap devre dışı bırakıldı.",
            )
        else:
            ...
            messages.success(
                request,
                f"{profile.user.get_full_name() or profile.user.username} onaylandı.",
            )
        log_event(
